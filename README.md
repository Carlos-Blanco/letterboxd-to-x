...
3. La subida de portadas usa `POST /2/media/upload` y publicar usa `POST /2/tweets`, ambos autenticados con el Bearer token de usuario. El workflow comprueba primero `GET /2/users/me` para distinguir un token inválido de un rechazo específico de subida de medios.
4. En `Settings → Actions → General`, permite que GitHub Actions lea y escriba en el repositorio para guardar el último elemento procesado.
5. Activa Actions y ejecuta manualmente **Letterboxd to X** una vez. Esa primera ejecución guarda la entrada más reciente como punto de partida sin publicarla. Las valoraciones posteriores se publicarán automáticamente mientras el access token siga vigente.
